...
public class RewriteViewTest extends AbstractCoreTest {
    ...
    public void testSimpleView() {
        ...
        transactional(new TxVoidWork() {
            @Override
            public void work(EntityManager em) {
                em.persist(new DocumentSimpleViewEntity(100L, "ABC"));
            }
        });

        // TODO: Not sure what happens, but currently, the materialization table is skipped?!
    ...
    public void testSimpleWithOwnerView() {
        ...
        transactional(new TxVoidWork() {
            @Override
            public void work(EntityManager em) {
                em.persist(new DocumentSimpleWithOwnerViewEntity(100L, "ABC", 200L, "DEF"));
            }
        });
        ...
        assertEquals(Long.valueOf(100L), results.get(0).getId());
        assertEquals("ABC", results.get(0).getName());
        assertEquals(Long.valueOf(200L), results.get(0).getOwner().getId());
        assertEquals("DEF", results.get(0).getOwner().getName());
    }
